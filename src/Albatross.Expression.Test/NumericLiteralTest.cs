...
namespace Albatross.Expression.Test {
	...
	public class NumericLiteralTest {
		...
		public string Tokenize(string expression, int start) {
			int next;
			var construct = typeof(NumericLiteral).GetConstructors(BindingFlags.NonPublic | BindingFlags.Instance).First();
			var token = (IExpression)construct.Invoke(new object[0]);
